...
const campaignFields = Object.fromEntries(["campaignId", "campaignName", "campaignSource", "campaignStart", "campaignEnd", "campaignReminderLeadDays", "campaignReminderTime", "campaignCards", "campaignRule", "campaignStatus"].map((id) => [id, document.querySelector(`#${id}`)]));
...
function nextMonthlyDate(day) { if (!day) return null; const now = today(); const year = now.getFullYear(); const month = now.getMonth(); let date = new Date(year, month, Math.min(Number(day), daysInMonth(year, month))); if (date < now) date = new Date(year, month + 1, Math.min(Number(day), daysInMonth(year, month + 1))); return date; }
function parseDate(value) { if (!value) return null; const d = new Date(`${value}T00:00:00`); return Number.isNaN(d.getTime()) ? null : d; }
function annualDate(value) { const d = parseDate(value); if (!d) return null; const now = today(); let next = new Date(now.getFullYear(), d.getMonth(), d.getDate()); if (next < now) next = new Date(now.getFullYear() + 1, d.getMonth(), d.getDate()); return next; }
...
function fmtDateTime(date) { if (!date) return "未设置"; return `${fmt(date)} ${String(date.getHours()).padStart(2,"0")}:${String(date.getMinutes()).padStart(2,"0")}`; }
function money(value) { const match = String(value || "").replaceAll(",", "").match(/\d+(\.\d+)?/); return match ? Number(match[0]) : 0; }
function directCashback(card) { const text = [card.cashbackType, card.cashbackRate, card.useTags].flat().join(" ").toLowerCase(); return /返利|返现|cashback|账单抵扣|现金/.test(text) ? 100 + money(card.cashbackRate) : 0; }
function cardScore(card) { let score = 0; const payDays = daysUntil(nextMonthlyDate(card.paymentDay)); if (payDays > 0 && Number.isFinite(payDays)) score += payDays; if (directCashback(card)) score += 20; if (card.status !== "正常" && card.status !== "备用") score -= 100; return score; }
...
function filteredCards() {
  ...
  if (els.sortMode.value === "cashback") cards = cards.filter((card) => directCashback(card));
  return cards.sort((a, b) => {
    ...
    if (mode === "cashback") return directCashback(b) - directCashback(a);
...
function makeReminder(type, date, title, meta, profile, notify = true) { return { type, date, title, meta, reminderAt: reminderDateTime(date, profile?.reminderLeadDays, profile?.reminderTime), notify }; }
function getReminders() {
  ...
  state.cards.forEach((card) => {
    ...
    if (card.replacement) reminders.push(makeReminder("换卡", today(), `${label} 新旧卡关联`, card.replacement, profile, false));
    if (card.riskStatus && card.riskStatus !== "无异常") reminders.push(makeReminder("异常", today(), `${label} ${card.riskStatus}`, card.refundTracking || card.disputeTracking || "需要跟进异常状态", profile, false));
  });
...
function renderUsage() { const scenarios=["大额","最长还款","返利","里程","境外","网购","酒店","机票","餐饮"]; els.scenarioList.innerHTML=scenarios.map((s)=>{ const list=[...state.cards].filter((c)=>c.status==="正常"||c.status==="备用").sort((a,b)=>{ if(s==="返利") return directCashback(b)-directCashback(a); if(s==="最长还款") return daysUntil(nextMonthlyDate(b.paymentDay))-daysUntil(nextMonthlyDate(a.paymentDay)); return (splitTags(b.useTags).includes(s)?1:0)-(splitTags(a.useTags).includes(s)?1:0); }).slice(0,3); return `<article class="scenario-card"><h3>${s}</h3><p>根据卡片档案字段筛选。</p>${list.map((c)=>`<div class="credit-mini"><strong>${esc(c.bank)} ${esc(c.name||"")} · ${esc(c.last4||"未填")}</strong><p>${esc(splitTags(c.useTags).join("；")||"备选")}</p></div>`).join("")}</article>`; }).join(""); }
function renderCampaigns() { els.campaignList.innerHTML = state.campaigns.length ? state.campaigns.map((c)=>`<article class="campaign-card"><div><h3>${esc(c.name)}</h3><p>${esc(c.source||"活动")} · ${esc(c.cards)}</p><p>${esc(c.rule)}</p><div class="tag-row"><span class="tag">${esc(c.start||"未设")} 至 ${esc(c.end||"未设")}</span><span class="tag">提醒：提前 ${reminderLeadDays(c.reminderLeadDays)} 天 ${esc(c.reminderTime || DEFAULT_REMINDER_TIME)}</span><span class="tag">${esc(c.status||"未用")}</span></div></div><div class="campaign-actions"><button class="small-btn" data-action="edit-campaign" data-id="${c.id}">编辑</button></div></article>`).join("") : `<div class="empty">暂无活动</div>`; }
function renderData() { els.dataPreview.value = JSON.stringify(state, null, 2); }
function render() { els.todayText.textContent = `${new Date().toLocaleDateString("zh-CN")} · APK提醒版 ${BUILD}`; renderSummary(); renderRecommendation(); renderCards(); renderReminders(); renderUsage(); renderCampaigns(); renderData(); saveState(); }
...
function openCampaignDialog(c=null) { campaignFields.campaignId.value=c?.id||""; campaignFields.campaignName.value=c?.name||""; campaignFields.campaignSource.value=c?.source||""; campaignFields.campaignStart.value=c?.start||""; campaignFields.campaignEnd.value=c?.end||""; campaignFields.campaignReminderLeadDays.value=c?.reminderLeadDays??""; campaignFields.campaignReminderTime.value=c?.reminderTime||DEFAULT_REMINDER_TIME; campaignFields.campaignCards.value=c?.cards||""; campaignFields.campaignRule.value=c?.rule||""; campaignFields.campaignStatus.value=c?.status||"未用"; els.campaignDialog.showModal(); }
function saveCampaign() { if(!campaignFields.campaignCards.value.trim()) return alert("请填写适用卡。"); const c={ id:campaignFields.campaignId.value||makeId(), name:campaignFields.campaignName.value.trim(), source:campaignFields.campaignSource.value.trim(), start:campaignFields.campaignStart.value, end:campaignFields.campaignEnd.value, reminderLeadDays:campaignFields.campaignReminderLeadDays.value===""?"":Number(campaignFields.campaignReminderLeadDays.value), reminderTime:campaignFields.campaignReminderTime.value||DEFAULT_REMINDER_TIME, cards:campaignFields.campaignCards.value.trim(), rule:campaignFields.campaignRule.value.trim(), status:campaignFields.campaignStatus.value }; const index=state.campaigns.findIndex((item)=>item.id===c.id); if(index>=0) state.campaigns[index]=c; else state.campaigns.unshift(c); render(); }

els.tabs.forEach((tab)=>tab.addEventListener("click",()=>setView(tab.dataset.view)));
els.searchInput.addEventListener("input",renderCards); els.statusFilter.addEventListener("change",renderCards); els.sortMode.addEventListener("change",renderCards);
...
els.addCardBtn.addEventListener("click",()=>openCardDialog()); els.addCampaignBtn.addEventListener("click",()=>openCampaignDialog());
...
document.addEventListener("click",(event)=>{ const button=event.target.closest("[data-action]"); if(!button)return; const {action,id}=button.dataset; if(action==="edit-card") openCardDialog(state.cards.find((card)=>card.id===id)); if(action==="delete-card"){ state.cards=state.cards.filter((card)=>card.id!==id); render(); } if(action==="add-campaign-for-card"){ const card=state.cards.find((item)=>item.id===id); if(card) openCampaignDialog({ cards:`${card.bank} ${card.name||""} 尾号${card.last4||"未填"}`, status:"未用" }); } if(action==="edit-campaign") openCampaignDialog(state.campaigns.find((c)=>c.id===id)); });
